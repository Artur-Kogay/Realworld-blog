...
const SettingsProfileForm = () => {
  ...
  const handleDataSubmit: SubmitHandler<ICreateArticleForm> = async (
    data,
  ): Promise<void> => {
    try {
      const formattedData = {
        ...data,
        tagList: formattedTags(data?.tagList) ?? [''],
      };
  ...
  return (
    <>
      ...
      <form className={styles.form} onSubmit={handleSubmit(handleDataSubmit)}>
        ...
        />
        {errors.body && (
          <p className={styles['error-message']}>{errors.body.message}</p>
        )}
        <Input
          type="text"
        ...
        />
        {errors.tagList && (
          <p className={styles['error-message']}>{errors.tagList.message}</p>
        )}
