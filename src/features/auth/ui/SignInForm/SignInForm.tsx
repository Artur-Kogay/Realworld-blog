...
const SignInForm = () => {
    ...
    const handleDataSubmit: SubmitHandler<ISignInForm> = async (data): Promise<void> => {
    ...
    return (
        <>
            <form className={styles.form} onSubmit={handleSubmit(handleDataSubmit)}>
                {errors.serverError && <p className={styles['error-message']}>{errors.serverError.message}</p>}
                <Input
                    type='email'
                    placeholder='Email'
