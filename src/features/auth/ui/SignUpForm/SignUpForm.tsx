...
const SignUpForm = () => {
    ...
    const handleDataSubmit: SubmitHandler<ISignUpForm> = async (data): Promise<void> => {
            ...
            <form className={styles.form} onSubmit={handleSubmit(handleDataSubmit)}>
                ...
                <Input
                    ...
                    {...register('email', {
                        ...
                        onChange: () => clearErrors(),
                        validate: (value) => /^(([^<>()[\].,;:\s@"]+(\.[^<>()[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/.test(value) || 'это не email'
                    })}
                />
                ...
                />
                {errors.repeatPassword && <p className={styles['error-message']}>{errors.repeatPassword.message}</p>}
                <label>
                    <input type="checkbox"
                        {...register('checkbox',
                            {
